...
def _log_delivery_failure(err, _msg) -> None:
    if err is not None:
        print(f"sample-service: Kafka delivery failed: {err}", file=sys.stderr, flush=True)
...
class KafkaProducer(ExamplePublisherPortOut):
    ...
    def publish(self, example: Example) -> None:
        ...
        if remaining > 0:
            print(
                f"sample-service: Kafka flush timed out with {remaining} message(s) undelivered",
                file=sys.stderr,
                flush=True,
            )
        print(f"[sample-service] published {len(data)} bytes to {self._topic}", flush=True)
