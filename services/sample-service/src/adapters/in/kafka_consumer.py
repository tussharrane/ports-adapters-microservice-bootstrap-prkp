...
class KafkaConsumer:
    def __init__(self, usecase: ProcessExampleUseCasePortIn) -> None:
        ...
        group_id = os.environ.get("KAFKA_GROUP_ID", "sample_service")
    ...
    def start(self, stop_event: threading.Event) -> None:
        while not stop_event.is_set():
            ...
            if msg is None:
                continue
            if msg.error():
                print(f"sample-service: consume error: {msg.error()}", file=sys.stderr, flush=True)
                continue
            value = msg.value()
            if value is None:
                continue
        ...
        print("sample-service: consumer closed cleanly", file=sys.stderr, flush=True)
